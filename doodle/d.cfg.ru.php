...
,	'new_room' =>	'Комната ещё не существует. Первый участник станет управляющим — модератором комнаты.'
);
$tmp_archive = 'Архив';
...
.'\\		Например так,
	...
	[cite|/ Например так, / или дважды / / для пустой строки. /]
	Результат показан справа. →
	Не отделённые полностью черты останутся как есть, включая [cite|//] двойные.
]';
...
$tmp_draw_limit_hint = 'Допустимо присылать рисунки, сделанные своими руками, размером от %sх%s до %sх%s пикселей и не более %s байт (%s) в файле одного из типов: %s.';
...
,	'check' => array(
	...
	,	'picprogress'	=> 'Показывать прогресс обработки поста с рисунком'
	...
	,	'capture_altclick'	=> 'Ctrl/Shift + клик: выделить посты, Alt + клик: сохранить снимок'
	,	'capture_textselection'	=> 'Выделить текст в постах, кнопкой рядом сохранить снимок'
	)
,	'input' => array(
